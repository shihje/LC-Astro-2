...
% User define params
DataDrive = '/Users/gabidrummond/Dropbox (MIT)/Sur lab stuff/LC_Astro';
AN = 'DbhAstro07';
FolderName = '20220525';
...
tStart = MTXResp(:,1) - MTXResp(1,1) + dataAstro.t2PHBehStart; %in the 2P imaging timeline
tStartBeh = MTXResp(:,1); %cue off in the behavior timeline

% Determine trial type
H = MTXTrialType(:,3) >= 5 & MTXResp(:,3) > 0;
M = MTXTrialType(:,3) >= 5 & MTXResp(:,3) < 1;
CR = MTXTrialType(:,3) < 5 & MTXResp(:,3) < 1;
FA = MTXTrialType(:,3) < 5 & MTXResp(:,3) > 0 & MTXTrialType(:,5) >0;
...
fore = MTXTrialType(:,4); %delay between cue and tone
%MTXResp(:,2)-MTXResp(:,1); %same as above
RT = MTXResp(:,4)-MTXResp(:,2); %reaction time, tone to press
