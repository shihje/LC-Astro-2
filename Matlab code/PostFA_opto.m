%% extract behavior for n+1 trials, incorporating opto vs no opto
...
idxOpto = dataBEH.params.MTXTrialType(:,5);


 if isfield(dataBEH.params, 'laser')
    idxOpto = dataBEH.params.MTXTrialType(:,5) == 1; %find laser trials
 else 
     idxOpto = false(size(spikes)); 
 end 
...
for i = 1:length(idx)
   if trialType(i) == 4 & trialType(i + 1) == 1 & idxOpto(i)==0
        trialTypepostFA(i+1) = 1;
   else if trialType(i) == 4 & trialType(i + 1) ==2 & idxOpto(i)==0
           trialTypepostFA(i+1) =2; 
       else if trialType(i)== 4 & trialType(i + 1) == 3 & idxOpto(i)==0
               trialTypepostFA(i+1) = 3; 
           else if  trialType(i)== 4 & trialType(i + 1) == 4 & idxOpto(i)==0
                   trialTypepostFA(i+1)=4;
               else if trialType(i) == 4 & trialType(i + 1) == 1 & idxOpto(i)==1
                    trialTypepostFA(i+1) = 5;
                   else if trialType(i) == 4 & trialType(i + 1) ==2 & idxOpto(i)==1
                        trialTypepostFA(i+1) =6; 
                       else if trialType(i)== 4 & trialType(i + 1) == 3 & idxOpto(i)==1
                            trialTypepostFA(i+1) = 7; 
                           else if trialType(i)== 4 & trialType(i + 1) == 4 & idxOpto(i)==1
                                trialTypepostFA(i+1)=8;
                                   end
                               end
                           end
                       end
...
% 
% % some figure set up
TITLE = {'Post FA- Hit','Post FA - Miss','Post FA- CR','Post FA- FA', 'Post FA opto- Hit', 'Post FA opto- Miss', 'Post FA opto- CR', 'Post FA opto- FA'};
c = setColor;
cA(:,1) = c.blue2;
cA(:,2) = [0 0 0];
cA(:,3) = c.purple;
...
%     
    for j = 1:8
        ...
        if j ==5
            mHOpto(:,i) = m;
            errHOpto(:,i) = m;
            peakNEHOpto(:,i) = peakNE(beh22PH(:,3) == j,i);
        end 
        if j == 7
            peakNEITICROpto(:,i) = peakNEITI(beh22PH(:,3) == j,i);
        end
        if j == 8
            mFAOpto(:,i) = m;
            errFAOpto(:,i) = err;
