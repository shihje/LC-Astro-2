%% astrocyte version of C_GRABNE_extractBehavior
...
nROI = size(dFF,2);
    ...
    % Create raster 2PH
    [A,R] = makeRaster(NE,idStart2PH(~isnan(idStart2PH)),round(win/fps));
    ...
    peakNE(:) = nanmax(R(:,tR2PH > winResp(1) & tR2PH < winResp(2)),[],2);
    peakNEITI(:) = nanmax(R(:,tR2PH > winITI(1) & tR2PH < winITI(2)),[],2);
    ...
    for j = 1:4
        [m, err] = mean_sem(R(beh22PH(:,3) == j,:));
        
        if j == 1
            mH(:,1) = m;
            errH(:,1) = err;
            peakNEH(:,1) = peakNE(beh22PH(:,3) == j);
        end
        if j == 3
            peakNEITICR(:,1) = peakNEITI(beh22PH(:,3) == j);
        end
        if j == 4
            mFA(:,1) = m;
            errFA(:,1) = err;
